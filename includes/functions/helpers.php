 ...
 * @return array
 */
function cs_plugin_info($plugin_slug) {
	add_filter('extra_plugin_headers', create_function('', 'return array("GitHub Plugin URI","Twitter");'));
	$plugin_data = get_plugin_data(CS_PATH . $plugin_slug . '.php');
 ...
 * @return type
 */
function cs_get_edit_link($dir_type, $directory, $filename) {
...
function cs_find_recursive_php_files($path) {
    $files = array();
    $fileNameArr = array();
...
function cs_scan_directory($root_dir, $code_injections) {
    ...
    if (count($filenames) > 0) {
        foreach ($filenames as $dir) {
            $directories[$dir] = cs_find_recursive_php_files($root_dir . "/" . $dir);
        }
